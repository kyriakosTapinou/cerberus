...
def plot2(frame, data, output_name):
    xn_rhoi, yn_rhoi, rhoi, rhoi_min, rhoi_max, label_rhoi, time = extractPlotData(frame, data, "rho-ions", r"$\rho_i$", getTime=True)
    ...
    yn, xn = np.meshgrid(yn_Ex, xn_Ex)

    axes = []

    fig = plt.figure(figsize=(16,9))
    #fig = plt.figure(constrained_layout=True)
    gs = gridspec.GridSpec(ncols=2, nrows=3) #, figure=fig)#, hspace=0.1)
    ...
    subPlot(axes, fig, gs, xn, yn, 2, 0, Ey, Ey_min, Ey_max, label_Ey)
    subPlot(axes, fig, gs, xn, yn, 1, 1, Bz, Bz_min, Bz_max, label_Bz)
    #subPlot(axes, fig, gs, xn, yn, 2, 1, Jx, Jx_min, Jx_max, label_Jx)
    ...
    # particles 
    i_px, i_py, i_pi = get_particle_trajectories(data["particles-ions"], limits)
    e_px, e_py, e_pi = get_particle_trajectories(data["particles-electrons"], limits)
    l = 0 #streak length
    axes[0].plot(i_px[-l::], i_py[-l::], "w-", lw=0.5, alpha=0.5)
    axes[0].plot(i_px[-1], i_py[-1], "wo", ms=0.5, alpha=0.5)
    axes[1].plot(e_px[-l::], e_py[-l::], "w-", lw=0.5, alpha=0.5)
    axes[1].plot(e_px[-1], e_py[-1], "wo", ms=0.5, alpha=0.5)
...
if 1:
    ...
    q["get"] = [
        ...
        {"func":get_particles, "tag":"particles-ions", "get_streak":True, "component":"ions"},
        {"func":get_particles, "tag":"particles-electrons", "get_streak":True, "component":"electrons"},
    ...
    q["plot"] = plot2
    q["name"] = "SRMI-Li3-option-19-Clean" # "SRMI-Li3-option-19-Ideal-Clean-J_E_B_rc"
    ...
    q["file_include"] = ["SRMI-Li3-Collisions-Braginskii.plt"]
    q["file_exclude"] = ["chk"]
    q["cores"] = 8
